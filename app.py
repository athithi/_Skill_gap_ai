...
def _tfidf_similarity(texts_a: list, texts_b: list) -> np.ndarray:
    """
    Hybrid similarity: blends three signals so the matrix shows a
    full range of values (not just 0 and 1).

    1. Char n-gram TF-IDF (2-4)  — catches surface similarity
    2. Word-level TF-IDF (1-2)   — catches shared content words
    3. Ontology category boost   — skills in the same domain get
                                   a floor score even with 0 lexical overlap
    Final = 0.45*char + 0.35*word + 0.20*category
    """
    ...
    # Signal 1: char n-gram
    char_vec = TfidfVectorizer(analyzer="char_wb", ngram_range=(2, 4), min_df=1)
    char_vec.fit(all_texts)
    sim_char = cosine_similarity(char_vec.transform(texts_a), char_vec.transform(texts_b))

    # Signal 2: word n-gram
    word_vec = TfidfVectorizer(analyzer="word", ngram_range=(1, 2), min_df=1)
    word_vec.fit(all_texts)
    sim_word = cosine_similarity(word_vec.transform(texts_a), word_vec.transform(texts_b))

    # Signal 3: ontology category overlap
    RELATED_CATS = {
        "Data Science & ML":          {"AI & Generative AI", "Data Engineering", "Mathematics", "Tools & Productivity"},
        "AI & Generative AI":         {"Data Science & ML", "Programming Languages"},
        "Data Engineering":           {"Data Science & ML", "Databases", "Cloud & DevOps"},
        "Cloud & DevOps":             {"Data Engineering", "Networking & Security", "Version Control"},
        "Embedded Systems & IoT":     {"Networking & Security", "Programming Languages"},
        "Networking & Security":      {"Cloud & DevOps", "Embedded Systems & IoT"},
        "Web Development":            {"Design & UI/UX", "Databases", "Version Control", "Mobile Development"},
        "Mobile Development":         {"Web Development", "Design & UI/UX"},
        "Teaching & Education":       {"Tools & Productivity", "Soft Skills"},
        "Mathematics":                {"Data Science & ML"},
        "Soft Skills":                {"Project Management & Agile", "Teaching & Education"},
        "Project Management & Agile": {"Soft Skills", "Business & Finance"},
        "Business & Finance":         {"Project Management & Agile", "Tools & Productivity"},
        "Design & UI/UX":             {"Web Development", "Mobile Development", "Tools & Productivity"},
        "Version Control":            {"Cloud & DevOps", "Web Development"},
        "Databases":                  {"Data Engineering", "Web Development", "Cloud & DevOps"},
        "Tools & Productivity":       {"Teaching & Education", "Business & Finance", "Design & UI/UX"},
        "Programming Languages":      {"Web Development", "Data Science & ML", "Embedded Systems & IoT"},
    }

    def cat_of(skill):
        return SKILL_TO_CAT.get(skill.lower(), "")

    n_a, n_b = len(texts_a), len(texts_b)
    sim_cat = np.zeros((n_a, n_b))
    for i, a in enumerate(texts_a):
        ...
        for j, b in enumerate(texts_b):
            ...
            if ca and cb:
                if ca == cb:
                    sim_cat[i, j] = 0.40
                elif cb in RELATED_CATS.get(ca, set()):
                    sim_cat[i, j] = 0.15

    combined = 0.45 * sim_char + 0.35 * sim_word + 0.20 * sim_cat
...
CANONICAL = {
    ...
    "parent teacher conference":"parent communication",
    "parent teacher conferences":"parent communication",
    "staff meeting":"staff meetings","digital tool":"digital tools",
    ...
    "cybersecurity security":"cybersecurity","embedded systems":"embedded systems",
...
def _is_name_word(w: str) -> bool:
    if not _NAME_WORD_RE.match(w): return False
    wl = w.lower()
    if not (w[0].isupper() or w.isupper()): return False
    return (wl not in SECTION_HEADERS and wl not in ALL_KNOWN_SKILLS and wl not in HARD_NOISE)
...
def extract_candidate_name(raw_text: str) -> str:
    for line in raw_text.splitlines()[:20]:
        ...
        for seg in segments:
            ...
            if not all(_is_name_word(w) for w in words): continue
            if re.search(r'[\d@\.:/]', seg): continue
...
def strip_name_from_text(raw_text: str, name: str) -> str:
    if not name: return raw_text
    name_lower = name.lower()
    kept = []
    for line in raw_text.splitlines():
        kept.append("" if name_lower in line.lower() else line)
    return "\n".join(kept)
...
def preprocess_for_nlp(text: str) -> str:
    ...
    for line in text.splitlines():
        ...
        if len(words) <= 4 and not any(c in s for c in ['.', ',', ';']):
            if words and words[0].lower() in SECTION_HEADERS: continue
        lines.append(s)
    text = ' '.join(lines)
...
def extract_structured_skills(text: str, blocked_tokens: set = None) -> set:
    ...
    for line in text.splitlines():
        ...
        if line_lower in SECTION_HEADERS: continue
        if _BOILERPLATE.search(line): continue
        if ':' in line:
            cat, _, items_str = line.partition(':')
            if cat.strip().lower() in SECTION_HEADERS: continue
            for item in items_str.split(','):
                item = re.sub(r'\([^)]*\)', '', item.strip()).strip()
                if not item or len(item) < 2 or re.search(r'\d', item): continue
                if '@' in item or '.com' in item: continue
                if item.lower() in blocked_tokens: continue
                norm = _normalize_one(item.lower())
                if norm and norm not in blocked_tokens: skills.add(norm)
        else:
...
def _tok_ok(tok, blocked_tokens: set = None) -> bool:
    blocked_tokens = blocked_tokens or set()
    txt = tok.text.lower()
    return (tok.pos_ in ("NOUN","PROPN","ADJ") and not tok.is_stop
            and txt not in HARD_NOISE and txt not in blocked_tokens
            and not _ALWAYS_BLOCK.match(txt) and len(txt) >= 3
            and not re.match(r'^\d+$', txt) and '.com' not in txt)

def _phrase_valid(tokens) -> bool:
    return (all(t.text.lower() not in HARD_NOISE and t.text.lower() not in SOFT_NOISE
                and not _ALWAYS_BLOCK.match(t.text.lower()) for t in tokens)
            and not any(t.pos_ == "VERB" for t in tokens))

def extract_nlp_skills(text: str, blocked_tokens: set = None) -> set:
    blocked_tokens = blocked_tokens or set()
    doc = nlp(text)
    ner_noise = set(); ner_text_blocks = set()
    for ent in doc.ents:
        if ent.label_ in ("PERSON","GPE","LOC","ORG","DATE","TIME","CARDINAL","ORDINAL",
                           "MONEY","PERCENT","QUANTITY","NORP","FAC","EVENT","WORK_OF_ART"):
            for t in ent: ner_noise.add(t.i)
    ...
    for chunk in doc.noun_chunks:
        ...
        if not tokens or len(tokens) > MAX_PHRASE_WORDS: continue
        if not _phrase_valid(tokens):
            for t in tokens: chunk_idx.add(t.i)
            continue
        phrase = " ".join(t.text.lower() for t in tokens)
        if phrase in blocked_tokens or phrase in seen: continue
...
def _normalize_one(phrase: str):
    ...
    if p.endswith('ies') and len(p) > 5 and ' ' not in p: p = p[:-3] + 'y'
    if (' ' not in p and p.endswith('s') and len(p) > 5
            and p not in _NO_STRIP and not re.match(r'^[a-z]{2,4}$', p)):
        singular = p[:-1]
        if not re.search(r'[aeiou][bcdfghjklmnpqrtvwxyz]{3,}$', singular): p = singular
    if p in HARD_NOISE: return None
    return p

def split_co_listed(phrase: str) -> list:
    words = phrase.split()
    if len(words) >= 2 and all(w in ALL_KNOWN_SKILLS for w in words): return words
    return [phrase]
...
def finalize_skills(raw: set, source_text: str = "", blocked_tokens: set = None) -> set:
    ...
    for s in raw:
        if not s: continue
        s = s.lower().strip()
        if re.search(r'\d', s) or '@' in s or '.com' in s or 'http' in s: continue
        if len(s) < 3 or s in HARD_NOISE or _ALWAYS_BLOCK.match(s) or s in blocked_tokens: continue
        normed.update(split_co_listed(s))
    normed = {s for s in normed if len(s.split()) <= 3 and s not in blocked_tokens}
    confirmed = {s for s in normed if s in ALL_KNOWN_SKILLS}
    if source_text:
    ...
    for longer in sorted(confirmed):
        ...
        for n in range(1, len(words)):
            if " ".join(words[:n]) in confirmed: to_remove.add(longer); break
    return confirmed - to_remove
...
def extract_skills_from_resume(raw_text: str, blocked_tokens: set = None, name: str = "") -> set:
    ...
    return finalize_skills(structured | nlp_skills, source_text=clean_text.lower(), blocked_tokens=blocked_tokens)
...
def categorize_skills(skills):
    ...
    for sk in skills:
        sl = sk.lower()
        (soft if sl in SOFT_SKILLS or any(s in sl for s in SOFT_SKILLS) else tech).add(sk)
    return tech, soft


# ══════════════════════════════════════════════════════════════════════════════
# SEMANTIC MATCH — TF-IDF, returns 5-tuple with full matrix
# ══════════════════════════════════════════════════════════════════════════════
def semantic_match(jd_skills, res_skills, threshold=0.60):
    """
    Returns: (matched, missing, extra, jd_list, res_list)
    Uses TF-IDF char-ngram similarity. Threshold lowered to 0.60
    (TF-IDF is less sensitive than BERT so needs a lower cutoff).
    Full N×M matrix is computed once and reused for matched pairs.
    """
    if not jd_skills or not res_skills:
        return [], list(jd_skills), list(res_skills), [], []
    jd_l = list(jd_skills)
    res_l = list(res_skills)
    sim = _tfidf_similarity(jd_l, res_l)   # ndarray shape (len(jd_l), len(res_l))
...
def compute_score(matched, jd_skills, jd_clean, res_clean):
    ...
    sim     = _tfidf_similarity([jd_clean[:2000]], [res_clean[:2000]])
...
def detect_domain(skills: set, raw_text: str) -> list:
    ...
    for domain, profile in DOMAIN_PROFILES.items():
        ...
        hits = sum(1 for k in kws if k in skills) * 2 + sum(1 for k in kws if k in text_lower)
        if hits > 0:
            scores[domain] = round(min(hits / len(kws) * 100, 99))
...
def extract_resume_insights(raw_text: str, res_skills: set, res_tech: list, res_soft: list) -> dict:
    ...
    # ── Years of experience ──
    years_exp = None
    for pat in [r'(\d+)\+?\s*years?\s*of\s*(?:work\s*)?experience',
                r'(\d+)\+?\s*years?\s*(?:experience|exp)',
                r'experience\s*(?:of\s*)?(\d+)\+?\s*years?']:
        m = re.search(pat, text)
        if m: years_exp = int(m.group(1)); break

    # ── Education level — checked in strict order (highest first) ──
    edu_level = "Not specified"
    # All checks use word boundaries to avoid substring false matches
    # e.g. "master" must not fire on "master data management"
    # e.g. "msc" must not fire inside "miscellaneous"
    if re.search(r'\b(ph\.d|phd|doctorate|doctoral)\b', text):
        edu_level = "PhD"
    elif re.search(r'\b(m\.tech|mtech|m\.e\.?|m\.sc\.?|msc|m\.s\.?|mba|m\.b\.a)\b', text):
        edu_level = "Postgraduate"
    elif re.search(r'\bmaster\b(?!\s+data)', text):   # "master" but NOT "master data"
        edu_level = "Postgraduate"
    elif re.search(r'\b(b\.tech|btech|b\.sc\.?|bsc|b\.com|bca|bachelor)\b', text):
        edu_level = "UnderGraduate"
    elif re.search(r'\bb\.e\.?\b', text):              # B.E. / B.E
        edu_level = "UnderGraduate"
    elif re.search(r'(?<![a-z])be(?![a-z])\s+\w*(electronics|computer|civil|mechanical|electrical)', text):
        edu_level = "UnderGraduate"                         # "BE Electronics..."
    elif re.search(r'\b(diploma|polytechnic)\b', text):
        edu_level = "Diploma"
    elif re.search(r'\b(12th|xii|higher secondary|hsc|plus two)\b', text):
        edu_level = "12th / HSC"

    # ── Cert + Project counting from raw bullet text ──
    _tail_section_re = re.compile(
        r'\s+(Mini\s+[Pp]rojects?|[Pp]rojects?|Certifications?|Certificates?|'
        r'Technical\s+Skills?|Soft\s+Skills?|Skills?|Education|Experience|'
        r'Work\s+Experience|Profile|Summary|Achievements?|'
        r'Extra\s+Curricular(?:\s+Activities)?|Activities|Paper\s+[Pp]resented|'
        r'Declaration|References?|Internship|Courses?|Training|Workshops?|'
        r'Languages?|Hobbies)\s*$'
    )
    _proj_hw = re.compile(
        r'\b(?:arduino|raspberry\s*pi|rfid|opencv|biometric|queue|stack|'
        r'data\s+struct|checkout|door\s+access|smart\s+vault|billing|'
        r'management\s+system|embedded\s+hardware|face.{0,5}recogni)\b',
        re.IGNORECASE
    )
    _action_split_re = re.compile(
        r'\s+(?=(?:Implemented|Designed|Integrated|Developed|Built|Created|'
        r'Used|Configured|Conducted|Performed|Achieved|Managed|Led|Worked|'
        r'Assisted|Supported|Presented)\b)'
    )
    _date_clean = re.compile(
        r'\s*\((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\w*\.?\s+20\d\d\)\s*$'
        r'|\s*[-–]\s*20\d\d\s*$', re.IGNORECASE
    )

    current_section = "header"
    cert_bullets, proj_bullets = [], []
    ...
    for chunk in chunks:
        ...
        if tail_m:
            ...
            new_section = re.sub(r'\s+', ' ', tail_m.group(1).strip().lower())
        else:
    ...
    certs, projs_from_cert = set(), set()
    for b in cert_bullets:
        title = _action_split_re.split(b, maxsplit=1)[0].strip()
        if not title or len(title) < 5: continue
        key = make_key(title)
        if not key or len(key) < 5: continue
        (projs_from_cert if _proj_hw.search(title) else certs).add(key[:80])
    ...
    for b in proj_bullets:
        ...
        if title and len(title) >= 5:
            key = make_key(title)
            if key and len(key) > 4: all_projs.add(key[:70])

    cats_covered = len({SKILL_TO_CAT.get(s) for s in res_skills if SKILL_TO_CAT.get(s)})
    breadth_pct  = round(cats_covered / len(SKILL_ONTOLOGY) * 100)
...
def run_pipeline(jd_raw, resume_raw):
    ...
    # semantic_match returns 5-tuple
    m_t, x_t, e_t, jd_t_ord, res_t_ord = semantic_match(jd_tech,  res_tech)
    m_s, x_s, e_s, jd_s_ord, res_s_ord = semantic_match(jd_soft,  res_soft)
    ...
    # ── Full N×M similarity matrix for the matrix panel ──
    full_matrix, full_jd_skills, full_res_skills = [], jd_t_ord, res_t_ord
    if jd_t_ord and res_t_ord:
        sim_matrix = _tfidf_similarity(jd_t_ord, res_t_ord)
        full_matrix = [
            [round(float(sim_matrix[j][r]), 3) for r in range(len(res_t_ord))]
            for j in range(len(jd_t_ord))
        ]
    ...
    return {
        ...
        "full_jd_skills":   full_jd_skills,
        "full_res_skills":  full_res_skills,
        "full_matrix":      full_matrix,
        "domains":          detect_domain(res_skills, resume_raw),
        "jd_domains":       detect_domain(jd_skills, jd_raw),
        "insights":         extract_resume_insights(
                                resume_raw, res_skills,
                                list(res_tech), list(res_soft)
                            ),
    }
